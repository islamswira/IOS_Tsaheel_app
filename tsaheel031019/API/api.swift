...
//        }
    ...
    //estraha details api call funcition
    class func getEstrahaDetails(EstrahaID: Int, completion: @escaping (EstrahaDetailsModel?, Error?)->Void){
        
        let url = "http://alc.com.sa/app/service/get_hotel_details/\(EstrahaID)"
...
//        ]
        ...
        Alamofire.request(url, method: .get, encoding: JSONEncoding.default, headers: headers)
            .responseJSON { response in
                ...
                case .success(let value):
                    do {
                        guard let data = response.data else { return }
                        let registerDataModel = try JSONDecoder().decode(EstrahaDetailsModel.self, from: data)
                        completion(registerDataModel, nil)
                    } catch let jsonError {
                        print(jsonError)
                    }
    ...
    // search for estraha based on time api call funciton
    class func getAvilableEstrahaBasedOntimeAnddate( city : String , dateIn : String , dateOut :String , timeIn: String , timeOut: String ,completion: @escaping (EstrahaDetailsModel?, Error?)->Void){
        
        let url = "http://alc.com.sa/app/service/hotel_check"
...
//       ]
        ...
        Alamofire.request(url, method: .post, parameters: parameter, encoding: JSONEncoding.default, headers: headers).responseString{response in
            
            print("response.result.debugDescription: \(response.result.debugDescription)")
            switch response.result
            {
            case .failure(let err):
                completion(nil,err)
                print("error on failure error returned : \(err)")
                let banner = Banner(title: "خطأ", subtitle: "مشكلة في الخادم ، حاول مرة اخرى !!", backgroundColor: UIColor(red:255.0/255.0, green:0/255.0, blue:0/255.0, alpha:1.000))
                banner.dismissesOnTap = true
                banner.show(duration: 3.0)
                
            case .success(let value):
                print("success and this is param : \(parameter)")
                do {
                    guard let data = response.data else { return }
                    print("data after success: \(data)")
                    let estrahaDataModel = try JSONDecoder().decode(EstrahaDetailsModel.self, from: data)
                    completion(estrahaDataModel, nil)
                } catch let jsonError {
